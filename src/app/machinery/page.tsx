...
export default function Page() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      if (json?.received) {
        setReceivedBookings(json.received.filter((b: any) => b.bookingType === 'MACHINERY' || !b.bookingType));
      }
    } catch (err) {
...
<div className="machinery-layout">
  ...
  {activeTab === 'find' && (
    <div className="tab-content active" id="findSection">
      ...
      {/* ── Incoming Bookings Notification Banner for Machine Owners ── */}
      {receivedBookings.length > 0 && (
        <div style={{
          marginBottom: '24px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontWeight: 800, color: '#14532d', fontSize: '1.05rem' }}>
                You have {receivedBookings.length} incoming equipment booking request{receivedBookings.length > 1 ? 's' : ''}!
              </div>
              <div style={{ fontSize: '0.85rem', color: '#15803d', marginTop: '2px' }}>
                Farmers have requested to rent your machinery. Click below to view customer name, phone number, and duration.
              </div>
            </div>
          </div>
          <button
          ...
          >
            View Incoming Requests ({receivedBookings.length}) →
          </button>
        </div>
      )}
  ...
  {/*  TAB 3: RECEIVED BOOKINGS (FOR MACHINERY OWNERS)  */}
  {activeTab === 'received' && (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h2 style={{ fontSize: '1.45rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>
              📥 Incoming Booking Requests for Your Machinery
            </h2>
            <p style={{ color: '#64748b', fontSize: '0.92rem', marginTop: '4px', margin: 0 }}>
              यहाँ आपकी रजिस्टर्ड मशीनों (ट्रैक्टर, हार्वेस्टर आदि) के सभी लाइव बुकिंग ऑर्डर्स दिखाई देंगे। किसान का नाम, फ़ोन नंबर, तारीख और समय देखकर ऑर्डर स्वीकार करें।
            </p>
          </div>
          <button
            onClick={fetchBookings}
        ...
        {receivedBookings.length > 0 ? (
            ...
            {receivedBookings.map((b) => (
              <div key={b.id} style={{
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                    <div>
                      <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#0f172a' }}>
                        🚜 {b.itemTitle || b.machinery?.title || 'Farm Machinery'}
                      </div>
                      <div style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '2px' }}>
                        📍 {b.district || b.machinery?.district || 'Nearby Area'}
                      </div>
                    </div>
                    <span style={{
                      fontSize: '0.78rem',
                  ...
                  {/* Customer Information (kisne book kiya) */}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: '#f0fdf4', borderRadius: '10px', marginBottom: '16px', border: '1px solid #bbf7d0' }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#166534' }}>Total Rental Earnings:</span>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  {b.status !== 'CONFIRMED' ? (
                    ...
                    >
                      {updatingBookingId === b.id ? 'Updating...' : '✅ Accept Booking'}
                    </button>
                  ) : (
                    <div style={{
                      flex: 1,
          ...
          <div style={{
            ...
            <p style={{ fontSize: '0.92rem', color: '#64748b', maxWidth: '440px', margin: '0 auto 20px', lineHeight: '1.5' }}>
              When farmers in your district book your registered equipment, their contact details, requested hours/duration, and booking dates will appear here immediately.
            </p>
            <button
              onClick={() => setActiveTab('register')}
              className="btn btn-primary cursor-pointer"
